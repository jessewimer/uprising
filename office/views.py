...
def germination_inventory_data(request):
    ...
    try:
        # Find the most recent germination year across both Germination and GermSamplePrint tables
        max_germ_year = Germination.objects.aggregate(
            max_year=Max('for_year')
        )['max_year']
        
        max_print_year = GermSamplePrint.objects.aggregate(
            max_year=Max('for_year')
        )['max_year']
        
        # Use the maximum of both tables, or default to 25
        actual_max_year = max(
            max_germ_year if max_germ_year is not None else 0,
            max_print_year if max_print_year is not None else 0
        )
        
        if actual_max_year == 0:
            actual_max_year = 25  # Default if no data in either table
            
        # Calculate the 4 germination years to display
        germ_years = []
        for i in range(3, -1, -1):  # 3, 2, 1, 0 (last 4 years)
            year = actual_max_year - i
        ...
        current_year = f"{actual_max_year:02d}"
        # The germ_year is the year we're currently working with for new prints
        germ_year = actual_max_year
        
        # print(f"Max years - Germ: {max_germ_year}, Print: {max_print_year}, Actual: {actual_max_year}")
        # print(f"Displaying years: {germ_years}, Current year: {current_year}, Germ year: {germ_year}")
        ...
        return JsonResponse({
            ...
            'current_year': current_year,  # The rightmost column year
            'germ_year': germ_year,  # The year for new prints (numeric)

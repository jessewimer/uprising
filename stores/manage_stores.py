...
def import_stores(dry_run=False):

    """
    Import stores from CSV.

    Args:
        dry_run (bool): If True, will only print what would happen without saving to DB.
    """
    full_file_path = os.path.join(os.path.dirname(__file__), "stores.csv")
    
    with open(full_file_path, newline='', encoding='utf-8') as csvfile:
        reader = csv.DictReader(csvfile)
        reader.fieldnames = [name.strip().replace('\ufeff', '') for name in reader.fieldnames if name.strip()]
        for row in reader:
            # Convert velcro to boolean
            velcro_value = row.get('velcro', '').strip().lower() in ('1', 'true', 'yes')

            store_num = int(row['store_num'])
            defaults = {
                'store_name': row.get('store_name', ''),
                'store_contact_name': row.get('store_contact_name', ''),
                'store_address': row.get('store_address', ''),
                'store_address2': row.get('store_address2', ''),
                'store_city': row.get('store_city', ''),
                'store_state': row.get('store_state', ''),
                'store_zip': row.get('store_zip', ''),
                'store_contact_phone': row.get('store_contact_phone', ''),
                'store_contact_email': row.get('store_contact_email', ''),
                'slots': int(row.get('slots', 0) or 0),
                'rack_material': row.get('rack_material', ''),
                'rack_num': row.get('rack_num', ''),
            ...
            if dry_run:
                try:
                    ...
                    print(f"[DRY RUN] Would update store {store.store_name}")
                except Store.DoesNotExist:
                    print(f"[DRY RUN] Would create store {row.get('store_name', '')}")
            else:
                ...
                print(f"{action} store {store.store_name}")


def assign_wholesale_pkt_products_to_stores():
    # Get all products ending in 'pkt' whose variety is wholesale
    ...
    for store in stores:
        for product in products_to_assign:
            # Create StoreProduct if it doesn't exist
            ...
            if created:
                print(f"Added {product.variety}{product.sku_suffix} to {store.store_name}")
            else:
                # Optionally update is_available if not True already
                if not obj.is_available:
                    obj.is_available = True
                    obj.save(update_fields=['is_available'])
                    print(f"Updated {product.variety}{product.sku_suffix} in {store.store_name} to available")

def reset_store_order_table():
    # Reset the store order table by deleting all entries
    StoreOrder.objects.all().delete()
    SOIncludes.objects.all().delete()
    print("Reset store order table")

# reset_store_order_table()
assign_wholesale_pkt_products_to_stores()
